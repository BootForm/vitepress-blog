...
<div v-for="post in posts" :key="post.url" class="mb-10 border-b border-black/10 pb-10 last:border-0 dark:border-white/10">
  ...
  <p class="mt-3 opacity-80">{{ post.excerpt }}</p>
  <div v-if="post.tags.length" class="mt-3 flex flex-wrap gap-2">
    <a
      v-for="tag in post.tags"
      :key="tag"
      :href="withBase(`/tags/${tag}.html`)"
      class="rounded-full bg-brand-50 px-2.5 py-0.5 text-xs font-medium text-brand-600 hover:bg-brand-500 hover:text-white dark:bg-brand-500/20 dark:text-brand-500"
    >#{{ tag }}</a>
  </div>
</div>
